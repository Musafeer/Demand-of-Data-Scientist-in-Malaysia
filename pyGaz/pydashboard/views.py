...
def dashboardView(request):
    ...
    df = pd.read_csv('https://raw.githubusercontent.com/Musafeer/Demand-of-Data-Scientist-in-Malaysia/main/pyGaz_EDA/final_dataset/companies_top20.csv')    
    fig = px.bar(df, x='jobs', y='companies', text_auto='.2s',color_discrete_sequence=px.colors.qualitative.Set3, orientation='h',
             title="Companies with the most jobs opening",width=1200, height=600)
    ...
    fig1 = px.bar(df1, x='jobs', y='companies', text_auto='.2s',color_discrete_sequence=px.colors.qualitative.Set1, orientation='h',
             title="Companies with the least jobs opening" ,width=1200, height=600)
    ...
    df3 = pd.read_csv('https://raw.githubusercontent.com/Musafeer/Demand-of-Data-Scientist-in-Malaysia/main/pyGaz_EDA/final_dataset/cleaned_is_remote2.csv')
    df3 = df3.head(10)
    fig3a = px.histogram(df3, y="location", x='num_is_remote',color="is_remote", orientation='h',width=1200, height=600)
    chart3a = fig3a.to_html()

    # Chart3b cleaned_is_remote2
    df3 = pd.read_csv('https://raw.githubusercontent.com/Musafeer/Demand-of-Data-Scientist-in-Malaysia/main/pyGaz_EDA/final_dataset/cleaned_is_remote2.csv')
    df3 = df3.tail(10)
    fig3b = px.histogram(df3, y="location", x='num_is_remote',color="is_remote", orientation='h',width=1200, height=600)
